...
5. **Close-before-ship gate** — the canonical pipeline order is gc-eop (close) before gc-ship. Using the same `{plan-slug}` resolved in item 4, glob for a session digest — `.construct/plans/{plan-slug}-session-digest_*.md` (in-project), or the matching legacy layout. If none exists, warn: 'No gc-eop digest found for plan `{plan-slug}` — gc-eop normally runs before shipping; consider running /gc-eop first.' Fail-open: advisory only, user may proceed (a preview/draft PR opened mid-milestone is a legitimate reason to ship before close).
