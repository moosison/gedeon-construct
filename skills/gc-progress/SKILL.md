...
`💰 Cumulative usage: {cumulative.sessions} sessions, ${cumulative.totals.estimatedCostUsd}` — followed by a caveat: "lags by the current session — folds in once a new session's first Stop event fires." If `cumulative.totals.unpriced` is `true`, append a second caveat: "partial — excludes tokens from one or more models with no pricing entry." Omit the line entirely if the file doesn't exist.
