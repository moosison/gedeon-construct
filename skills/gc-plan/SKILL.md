...
**Metadata-manifest sweep (counts, versions, names):** When a step renames a symbol or edits a hardcoded count, version, or project name that appears in metadata, the blast-radius sweep must explicitly enumerate the full manifest set — `package.json`, `.claude-plugin/*.json`, `README`, and any `marketplace.json` — and check each for the value *family*, not one file's exact phrasing. A search tuned to a single file's wording silently misses siblings carrying the same value: a README count bump can freshly stale `package.json` in the very same diff, shipping a self-contradiction, and a sibling may already be stale before the change starts. Prefer making such values count-agnostic (drop the brittle number) so the drift class cannot recur, and never assert a file "has no such string" without having searched it directly. (Caught at gc-review, gc-uiux-suite 2026-07-17: the sweep fixed `marketplace.json` but missed `package.json` + `plugin.json`, and the plan falsely claimed `package.json` carried no count string. See `feedback_blast_radius_sibling_manifests`.)
...
**Persona-contract completeness check:** When a plan step introduces or modifies a persona/agent (`agents/*.md`), or reuses an existing persona for a structurally different job than its established callers (a new lens, a new call shape, a materially different Input/Output usage), alongside the skill site that dispatches it, enumerate the persona's Input Contract items and Output Contract sections, and for EACH, name the exact dispatch-site instruction that supplies that input and the exact binding-site destination that consumes that output. An output section with no binding destination is silently dropped once the user confirms it; an input the dispatch never passes leaves the worker running against half the contract it was wired to expect. This is a cross-artifact check — validating the pieces individually (persona frontmatter valid, dispatch row parses, tier-check reports the persona CHECKED) does not catch it. (A HIGH orphan-output — `gc-inception-researcher`'s "Architecture Shape" section had no `REQUIREMENTS.md` destination — plus a MEDIUM unpassed-input — team size never reached the dispatch — both shipped past 4 preflight rounds in `vision-and-research-capture` (2026-07-19), caught only at `gc-review`'s Principal lens. Recurred a second time via reuse rather than authoring — `agents/gc-explorer.md` widened for two new lenses in `run-scenario-coverage` (2026-07-20) without triggering this check, since no persona file was introduced or modified. See `feedback_persona_contract_completeness`.)

**Skill hand-off continuity check:** When a plan step rewrites one skill's dispatch/routing instruction to invoke another skill inline, with a stated continuation ("...then proceed to X", "...then continue with Y"), read the invoked skill's own hand-off/exit instructions directly and verify they actually return control to X for that invocation path — not just that the invoked skill produces well-formed output. This is distinct from the Persona-contract completeness check above: that check covers Input/Output Contract fields between a plan step and a dispatched agent/persona; this one covers narrative hand-off continuity between two SKILL FILES' own prose when one is rewritten to invoke the other. An invoked skill authored to always route to its own default "next stage," with no branch for being invoked mid-flow from a different caller, silently breaks the calling instruction's own promise. (Confirmed live: `gc-brainstorm-skill-in-house-first-doctrine`, 2026-07-21 — `gc-bootstrap`'s rewritten Intent Gate row promised "...then proceed to scanning," but `gc-brainstorm`'s own hand-off step unconditionally routed to `/gc-discuss`/`/gc-plan`, skipping it. Missed by 2 full preflight rounds — 4 auditors + a lean auditor each round — caught only at code review by two independently-converging reviewer lenses.)
...
**Boundary-parsing precision check (host/domain matching, polymorphic event types):** When a step involves host/domain validation or classification, specify hostname-exact-or-subdomain matching (`hostname == domain or hostname.endswith("." + domain)` after `urlparse`), never raw substring containment (`if domain in url`) — substring matching misclassifies a spoofed domain (e.g. `evil.com/real-domain.com`) as legitimate. When a step involves a handler for a polymorphic/union event or update type, cross-check which sub-type variants the handler's own filter/subscription actually admits, and use the field guaranteed present across all of them rather than the narrowest-case field (e.g. `update.effective_message` over `update.message` for a Telegram handler whose subscription admits edited-message and channel-post variants where `.message` is `None`). Contributed upstream via `gc-contribute` (GitHub issue #1) after both bug classes were caught at round-1 pre-flight on a Telegram-bot allowlist/link-classification plan and verified fixed by this same addition in that project's local copy.
