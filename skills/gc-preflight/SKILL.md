...
// 9. [Pattern]: Phase-record upsert at the pipeline-state write — .construct/pipelines/{slug}.json, canonical mechanism (incl. slug validation and sessionId sourcing) in gc-plan's Pipeline state blockquote.
...
**Ordering (verify before revising):** complete this citation-verification cycle for ALL auditors before applying any in-round revision to the plan file. An orchestrator edit made between an auditor's read of the plan and this verification shifts or rewrites the cited lines, self-inflicting FAIL results that masquerade as auditor transcription errors. (Confirmed live, plan-artifact-relocation round 2, 2026-07-12: two auditors' plan citations failed purely because the orchestrator applied one auditor's own fixes before running verify-citations.) **Recurred a second time** (`construct-hygiene-instrument-repairs-ii`, 2026-07-20) under a specific, identifiable pressure this rule didn't previously name: a genuinely substantive HIGH finding was found mid-round, and the pull to fix it immediately — before the remaining auditors' citations were verified — overrode the ordering rule despite it already being on record. The fix itself was correct; only the ordering was wrong, and it was wrong a second time despite a standing memory note. **When this specific pressure hits** (a real, non-cosmetic finding surfaces before all auditors' citation checks are done): write the fix down as a planned edit, but hold the actual file write until every dispatched auditor's scratch-file citation cycle (steps 1-5 above) has completed — the few minutes' delay is cheaper than a self-inflicted FAIL that then has to be manually distinguished from a real auditor error at report-write time.

**Re-dispatch format feedback:** when a prior round's citation verification produced FAIL rows for an auditor, that lens's next-round dispatch prompt must LEAD with the concrete failure record from that auditor's own last output — count and shape (e.g. "your round-1 output failed 10/10 mechanical verification on format alone") — never merely restate the canonical rules. (Empirical, phase-scoped-pipeline-json 2026-07-13: five novel drift shapes survived complete format rules present in every dispatch prompt across two rounds; the one auditor given a quantified callout returned 7/7 PASS.) And when a round ends with zero substantive BLOCKER/HIGH findings and its Gate: STOP rests solely on citation-format failures whose underlying claims the orchestrator has manually verified against files read directly, the sanctioned close is a direct orchestrator re-verification round — no auditor panel, zero auditor citations, so this step passes trivially and Step 2.6 re-runs as normal (precedent: plan-store-project-namespacing Round 8; reused phase-scoped-pipeline-json Round 3) — not another full panel re-run purely to regenerate well-formatted citations.
