...
Look for two categories: absolute local file paths (a `C:\Users\{name}\`, `/home/{name}/`, or `/Users/{name}/` prefix), and secret-shaped tokens (a contiguous run of 20 or more alphanumeric characters with mixed case or digits, or a recognizable credential prefix such as `sk-`, `ghp_`, `AKIA`). Default to redacting anything detected in either category — replace it with a visible generic placeholder (`{local-path}` or `{redacted-token}`) that stays in the presented draft, so the user can see exactly what was pulled and why. Only restore the original if the user explicitly asks for it, before confirming.

Both categories default to redact, not just paths. Letting a real secret pass through onto a permanent public surface is a materially worse outcome than over-redacting a benign-but-secret-shaped string — and the second mistake costs the user nothing more than asking to have it put back.
