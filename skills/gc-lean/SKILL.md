...
- **Citation surface and scope.** The Citation column added to `agents/gc-lean-auditor.md`'s YAGNI Step Verdict Table (see `agents/gc-lean-auditor.md`) is the canonical, mechanically-verified citation surface — this is what `/gc-preflight`'s citation-verification step (`gc-preflight/SKILL.md`) extracts from via `extractCitations`. The DELETE-verdict citation rule stated above is narrower in scope (DELETE verdicts only) and predates the Citation column; it is superseded, not contradicted, by the column requirement, which covers every verdict type. When `gc-lean` is dispatched as Auditor D from inside `/gc-preflight`, its Citation-column findings are mechanically verified as part of that step. When `gc-lean` is run standalone, this mechanical verification does not apply — the citation instructions above are advisory only in that mode.
