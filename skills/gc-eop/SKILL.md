...
- Your active time: `{time.humanActiveSeconds}` formatted as `Xm Ys` (since the current milestone's plan-slug became active — may be shorter than true OS-session wall time if a mid-session milestone transition occurred; see the note above `### Session Usage`)
- Claude's build time: `{time.claudeBuildSeconds}` formatted as `Xm Ys` (same scoping as above)
- Idle (excluded): `{time.idleSeconds}` formatted as `Xm Ys` — omit this bullet entirely if the value is `0`, to avoid noise on short sessions with no stalls
...
- Milestone-scoped total: resolve the active plan-slug using this skill's own established fallback chain (`.claude/gc-pipeline.json`'s `slug` → `ARGUMENTS` → plan frontmatter `name:` → filename stem). If resolved, read `.construct/USAGE-BY-PLAN-SLUG.json`'s `[{resolved-slug}]` directly (a fresh read, separate file from `USAGE.json`; may be absent — treat as a zero bucket, not an error). Render "This milestone: {totalTokens} tokens, ${estimatedCostUsd} (formatted to 4 decimal places)" from `byPlanSlug[{resolved-slug}].totals` alone. If the slug doesn't resolve, or there is no matching bucket, omit this line silently — never render a zero or an error. Also render "Time on this milestone: {humanActiveSeconds} active, {claudeBuildSeconds} Claude build" (formatted `Xm Ys` each) from `byPlanSlug[{resolved-slug}].time` — same zero-bucket-if-absent handling as the tokens/cost line above, omit silently if the slug doesn't resolve.
- If `USAGE.json`'s `cumulative.totals.estimatedCostUsd` exceeds the sum of every bucket in `.construct/USAGE-BY-PLAN-SLUG.json`, that gap is expected, not a bug — a documented mix of pre-2026-07-08 spend (before per-milestone attribution existed in the code) and a one-time 2026-07-10 migration freeze, both permanent and non-recoverable. Do not attempt to reconcile it; do not render it unless directly asked.
...
3. **If git repo exists and uncommitted changes exist:**
   ...
   - **Already-merged reconciliation (before branch creation):** when the branch check finds `feature/{plan-slug}` absent, distinguish "never created" from "created, merged, and deleted" before treating absence as license to create it fresh. If the base branch's history already contains this slug's merge (a `git log {base}` subject naming `feature/{plan-slug}` — the conventional merge-commit shape — i.e. a user-directed "merge then finalize" ran the merge before this close), the feature has already shipped. Do **not** re-create and push `feature/{plan-slug}` as if shipping fresh (a second PR for merged work), and do **not** emit the `/gc-ship` NEXT STEP. Instead treat any remaining tracked changes as post-merge housekeeping (eop bookkeeping, an in-session skill patch): commit them on a distinctly-named housekeeping branch (e.g. `chore/{plan-slug}-close`) — never on the base branch — which is then what `/gc-ship` would PR, scoped as close-out, not the feature. If no tracked changes remain (bookkeeping is gitignored `.construct/` only), note the already-shipped state and skip the commit. If the signal is ambiguous (on the base branch but no merge found — possibly a fresh, never-branched pipeline that simply skipped branching), surface both readings and let the user direct rather than guessing.
