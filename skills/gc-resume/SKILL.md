...
// 4. [Pattern]: Confidence extraction from Pre-Flight-Review: look for "**Overall: {N}%" pattern; if absent, display "confidence: unknown".
...
| `pre-flight` | read latest Pre-Flight-Review; ≥90% → gc-execute; <90% → fix + gc-preflight |
...
| `{slug}-Pre-Flight-Review_*.md` | ≥90% → gc-execute; <90% → fix + gc-preflight |
...
- If preflight exists: latest `{slug}-Pre-Flight-Review_*.md` — extract `**Overall: {N}%` pattern; display "confidence: unknown" if absent
...
### Pipeline Progress
- /gc-plan: ✅ {overview}
- /gc-preflight: {✅ {N} run(s), last: {X}% {verdict} | ⏳ in progress | ⬜ not run}
